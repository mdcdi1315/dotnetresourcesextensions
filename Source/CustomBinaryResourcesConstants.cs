...
namespace DotNetResourcesExtensions.Internal
{
    ...
    internal sealed class CustomBinaryResource
    {
        ...
        {
            if (str.CanWrite == false) { throw new ArgumentException("Cannot use an unwriteable stream for writing data."); }
        ...
        {
            if (str.CanRead == false) { throw new ArgumentException("Cannot use an unreadable stream for reading it's data."); }
            CustomBinaryResource ret = new();
            ret.rwstream = str;
            return ret;
        }
        ...
        private System.Byte[] ReadUntilHeaderEnd()
        {
            ...
            if (final == -1) { throw new CustomBinaryFormatException("Failed to detect header end.", ParserErrorType.Deserialization); }
        ...
        public BinResourceBlob ReadResource(System.Int64 position , CustomFormatter.ICustomFormatter fmt)
        {
            ...
            {
                throw new CustomBinaryFormatException("Invalid header start. Header must begin with RESOURCE." , ParserErrorType.Deserialization);
            }
            if (StringMatchesThis($"{CustomBinaryResourcesConstants.ResourceName}{CustomBinaryResourcesConstants.ValueStartChar}") == false)
            {
                throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the RESOURCENAME phrase.", ParserErrorType.Deserialization);
            }
            ...
            {
                throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the HEADERVERSION phrase.", ParserErrorType.Deserialization);
            }
            ...
            if ((v = ReadNextNumericValue()) > CustomBinaryResourcesConstants.CurrentHeaderVersion)
            {
                ...
                throw new CustomBinaryFormatException($"Invalid header. Header version is {v} , which cannot be read by this reader.", ParserErrorType.Versioning);
            }
            ...
            {
                throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the DOTNETTYPE phrase.", ParserErrorType.Deserialization);
            }
            ...
            {
                throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the RESOURCETYPE phrase.", ParserErrorType.Deserialization);
            }
            ...
            {
                throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the RESOURCESIZE phrase.", ParserErrorType.Deserialization);
            }
            ...
            {
                throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the VALUE phrase.", ParserErrorType.Deserialization);
            }
    ...
    internal sealed class CustomBinaryHeader
    {
        ...
        {
            if (rwstream.CanRead == false) { throw new ArgumentException("Cannot use an unreadable stream for reading it's data."); }
            ...
            {
                throw new CustomBinaryFormatException("Cannot read the resource header. It's magic value is invalid.", ParserErrorType.Deserialization);
            }
            if (EnsureHeaderEnd() == false) { throw new CustomBinaryFormatException("Corrupted resource header format detected.", ParserErrorType.Deserialization); }
            if (StringMatchesThis($"{CustomBinaryResourcesConstants.Version}{CustomBinaryResourcesConstants.ValueStartChar}") == false)
            {
                throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the VERSION phrase.", ParserErrorType.Deserialization);
            }
            ...
            if ((v = ReadNextNumericValue()) > CustomBinaryResourcesConstants.CurrentHeaderVersion)
            {
                ...
                throw new CustomBinaryFormatException($"The format version is invalid: {v} . The reader can only read up to {CustomBinaryResourcesConstants.CurrentFormatVersion}.", ParserErrorType.Versioning);
            }
            ...
            {
                throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the CURRENTFORMATS phrase.", ParserErrorType.Deserialization);
            }
            ...
            {
                throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the SUPPORTEDHEADERVERSION phrase.", ParserErrorType.Deserialization);
            }
            if ((v = ReadNextNumericValue()) > CustomBinaryResourcesConstants.CurrentFormatVersion)
            {
                throw new CustomBinaryFormatException($"Invalid header format version: {v} . This reader can only read up to version {CustomBinaryResourcesConstants.CurrentFormatVersion} .", ParserErrorType.Versioning);
            }
            ...
            {
                throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the DATAPOSALIGNMENT phrase.", ParserErrorType.Deserialization);
            }
            if ((v = ReadNextNumericValue()) != CustomBinaryResourcesConstants.DataPositionsAlignment)
            {
                throw new CustomBinaryFormatException("Invalid header. The specified data position alignment is possibly corrupted. This header stream is untrusted.", ParserErrorType.Deserialization);
            }
            ...
            if (ret.Version == 1) // Version 1
            {
                ...
                {
                    throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the DATAPOSITIONS phrase.", ParserErrorType.Deserialization);
                }
                ...
                while (found == false && rwstream.Position < rwstream.Length)
                {
                    ...
                    positions.Add(System.BitConverter.ToInt64(temp, 0));
            ...
            } else if (ret.Version == 2) // Version 2
            {
                ...
                {
                    throw new CustomBinaryFormatException("Invalid header. Header must exactly contain the DATAPOSITIONSCOUNT phrase.", ParserErrorType.Deserialization);
                }
                v = ReadNextNumericValue();
